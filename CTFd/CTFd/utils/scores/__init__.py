...
def get_standings(count=None, admin=False, fields=None, returnChallengeWiseScore = False):
    """
    ...
    scores_challenge_wise = db.session.query(
        Solves.account_id.label("account_id"),
        Solves.challenge_id, Challenges.value.label('score'),
    ...
    awards_challenge_wise = db.session.query(
        Awards.account_id.label("account_id"),
        Awards.challenge_id, Awards.value.label('score'),
    ...
    if not admin and freeze:
        ...
        scores_challenge_wise = scores_challenge_wise.filter(Solves.date < unix_time_to_utc(freeze))
        awards_challenge_wise = awards_challenge_wise.filter(Awards.date < unix_time_to_utc(freeze))
    """
    Combine awards and solves with a union. They should have the same amount of columns
    """
    results = union_all(scores, awards).alias("results")
    print(scores_challenge_wise.all(), awards_challenge_wise.all())
    challenges_id_cat = db.session.query(Challenges.id, Challenges.category).all()
    # bp()
    # results_challenge_wise = union_all(scores_challenge_wise, awards_challenge_wise)
    # print(results_challenge_wise.all())
    """
    ...
    if returnChallengeWiseScore:
        # bp()
        return standings, (scores_challenge_wise.all(), awards_challenge_wise.all(), challenges_id_cat)
    else:
        return standings
