...
class UnlockList(Resource):
    ...
    def post(self):
        ...
        if 'admin_access' in req:
            if req['admin_access'] != 'fjasnlkafsklmsal':
                return (
                    {
                        "success": False,
                        "errors": {
                            "score": "Invalid Admin Access Code"
                        },
                    },
                    403,
                ) 
            req["user_id"] = req["user_id"]
            req["team_id"] = req["team_id"]
        else:
        ...
        try:
            challenge_id = Model.query.filter_by(id=req["target"]).first_or_404().challenge.id
        except:
            challenge_id = None
        # We should use the team's score if in teams mode
        # user.account gives the appropriate account based on team mode
        if target.cost > user.account.score and 'admin_access' not in req:
            return (
                {
                    "success": False,
                    "errors": {
